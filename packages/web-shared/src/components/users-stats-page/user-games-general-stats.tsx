...
export const UserGamesGeneralStats: FC<UserGamesGeneralStatsProps> = memo(({ userId }) => {
  const { data: gamesDurationStats } = trpc.stats.getUserGamesDuration.useQuery(
    {
      userId,
    },
    {
  ...
  const { data: ratio } = trpc.stats.getUserRatio.useQuery(
    { userId },
    {
      initialData: {
        wins: 0,
